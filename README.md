...
一个**全程离线**的桌面图片处理软件，带图形界面。支持 Windows 和 macOS（Apple Silicon）。
...
| 平台 | 文件 | 说明 |
|---|---|---|
| **Windows** | `图片处理-1.0.0-安装包.exe` | 双击安装，带开始菜单和桌面快捷方式 |
| **Windows** | `图片处理-1.0.0-便携版.zip` | 解压即用，免安装 |
| **macOS** | `图片处理-1.0.0-mac-arm64.dmg` | 打开后把图标拖进「应用程序」 |
| **macOS** | `图片处理-1.0.0-mac-arm64.zip` | 解压得到 `.app`，拖到哪都能用 |

> macOS 版仅支持 **Apple Silicon（M 系列芯片）**，不支持 Intel Mac。

### 首次打开

因为本项目没有购买苹果/微软的商业签名证书，第一次打开时系统会拦一下：
...
| **高级** | 自动增强、边缘检测（Canny 阈值实时可调）、人脸检测（灵敏度、最小尺寸、框颜色、线宽、是否显示置信度和五官关键点均可调） |
...
| macOS | macOS 11 或更高，Apple Silicon（M 系列） |
...
本软件打包时包含了若干第三方开源组件（详见下方「第三方组件」）。这些组件按其**各自的原许可**提供，可能附带额外条款，且**同样不提供任何担保**。使用本软件时你应一并遵守这些许可。
...
> **注意：PyInstaller 不支持交叉编译，不能在 Windows 上生成 Mac 版。**
> 要么在一台 Mac 上打包，要么用下面的云端方式。

本机打包：

```bash
chmod +x build_mac.sh
./build_mac.sh            # M 系列芯片自动按 arm64 打包
```
...
python -m PyInstaller --noconfirm --clean --windowed --onedir \
  --name "OpenCV图片处理" --icon app.ico \
  --add-data "app.ico;." --add-data "app_icon.png;." \
  --add-data "models/face_detection_yunet_2023mar.onnx;models" --noupx main.py
"C:\Program Files (x86)\Inno Setup 6\ISCC.exe" 安装包.iss
```

### 用 GitHub Actions 云端打包 macOS 版（不需要本机装环境）

因为这个仓库里带着 macOS 的打包配置，可以直接用 GitHub 的云端 Mac 来构建：

- 往 `main` 分支推代码，会自动跑一次
- 或者在仓库 **Actions** 页面点 **Run workflow** 手动触发

跑完在运行页面的底部 **Artifacts** 里下载 `mac-arm64`，里面就是 `.dmg` 和 `.zip`。

> Artifacts 有 90 天保留期，过期后重新跑一次即可。

### 自检

`selftest.py` 是开发期的自检脚本，覆盖调色、滤镜、格式读写、人脸检测参数、批量任务控制、画布居中、多线程安全等 34 项：
...
OpenCV 自身还内含多个第三方库（libjpeg-turbo、libpng、libtiff、libwebp、FFmpeg 等），
各自的许可文本见 OpenCV 官方发行包的 `etc/licenses` 目录。
...
安装包.iss               Windows 的 Inno Setup 安装包配置
requirements.txt         依赖清单
LICENSE                  MIT 许可
models/                  人脸检测模型
.github/workflows/       云端打包与自动发布配置
```
